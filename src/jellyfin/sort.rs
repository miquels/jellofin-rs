        ...
        for field in &sort_fields {
            ...
            let ordering = match field_lower.as_str() {
                ...
                "datecreated" => {
                    ...
                    let a_date = a.date_created.as_ref().and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok());
                    let b_date = b.date_created.as_ref().and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok());
                    a_date.cmp(&b_date)
                }
                "premieredate" => {
                    let a_date = a.premiere_date.as_ref().and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok());
                    let b_date = b.premiere_date.as_ref().and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok());
                    a_date.cmp(&b_date)
                }
                "productionyear" => {
                    a.production_year.cmp(&b.production_year)
                }
                "sortname" => {
                ...
                "runtime" => {
                    a.runtime_ticks.cmp(&b.runtime_ticks)
                }
                "playcount" => {
                ...
                "dateplayed" => {
                    let a_date = a.user_data.as_ref()
                        .and_then(|d| d.last_played_date.as_ref())
                        .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok());
                    let b_date = b.user_data.as_ref()
                        .and_then(|d| d.last_played_date.as_ref())
                        .and_then(|s| chrono::DateTime::parse_from_rfc3339(s).ok());
                    a_date.cmp(&b_date)
                }
                ...
                "parentindexnumber" => {
                    a.parent_index_number.cmp(&b.parent_index_number)
                }
