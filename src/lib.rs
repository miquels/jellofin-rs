...
pub async fn run(config_path: &str, debug_logs: bool) -> Result<(), ServerError> {
    ...
    let collection_repo =
        Arc::new(collection::CollectionRepo::new().map_err(|e| {
            ServerError::Server(format!("Failed to create collection repo: {}", e))
        })?);

    for coll_config in &config.collections {
        collection_repo
            .add_collection(coll_config)
            .await
    ...
    let image_resizer = Arc::new(
        util::ImageResizer::new(cache_dir)
    ...
    if has_tls {
        ...
        let tls_config = {
            let certs = load_certs(cert_path)
                .map_err(|e| ServerError::Server(format!("Failed to load certs: {}", e)))?;
            let key = load_private_key(key_path)
                .map_err(|e| ServerError::Server(format!("Failed to load key: {}", e)))?;
