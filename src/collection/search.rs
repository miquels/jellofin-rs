...
impl SearchIndex {
    ...
    pub async fn rebuild(&self, collections: &HashMap<String, Collection>) -> Result<(), SearchError> {
    ...
    pub fn search(&self, query_str: &str, limit: usize) -> Result<Vec<SearchResult>, SearchError> {
        ...
        for (_score, doc_address) in top_docs {
            ...
            if let (Some(id), Some(collection_id), Some(item_type)) = (
                retrieved_doc.get_first(self.id_field).and_then(|v| v.as_str()),
                retrieved_doc.get_first(self.collection_id_field).and_then(|v| v.as_str()),
                retrieved_doc.get_first(self.item_type_field).and_then(|v| v.as_str()),
            ) {
    ...
    pub fn find_similar(&self, item_id: &str, limit: usize) -> Result<Vec<SearchResult>, SearchError> {
        let searcher = self.reader.searcher();
